...
let player, game = false, tileSize = 50, timer = 30, tornadoStrength = 15;
let chaserHealth = 25, time = 0, stage = 0, currentTiles, weatherText;
let movementSpeed = 10, spawnSuccess = true, attemptingSpawn;
let stormChaser, spawnAttempt = false, spawnX, spawnY, enemies = [];
let maxEnemies = 10, lastDamageTime = 0, damageCooldown = 100;
...
function how_PlayButton() {     // to click the how to play button
    if (moveToHTP == false) {
      if (runMM){
...
function HowToPlay() {   // loads how to play sprites (menu)
    information = new Sprite(width / 2, height / 2.25, width / 1.4, height / 1.4, "n");
...
function goBack() {           // goback function to main menu
    information.remove();
    backButton.remove();
    moveToHTP = false;
    mainMenu();
...
function mainGame() {
    if (stage == 1) {
      ...
      spriteParent2 = new Group();
      spriteParent2.collider = "s";
      spriteParent2.w = tileSize;
      spriteParent2.h = tileSize;
  
      borderWave = new spriteParent2.Group();
      borderWave.tile = "b";
      borderWave.w = tileSize;
      borderWave.h = tileSize;
      borderWave.visible = false;

      road = new spriteParent2.Group();
      road.tile = "r";
      road.w = 51;
      road.h = 51;
      road.debug = false;
...
function spawnEnemies() {
  if (gameOver || !game|| stage !== 2) return; // Exit if the game is over or paused
  ...
  for (let i = 0; i < enemiesToSpawn; i++) {
      ...
      while (!spawnAttempt) {
          ...
          if (tilemap2[spawnY][spawnX] === 'r') {
              let canvasX = spawnX * tileSize + tileSize / 2;
              let canvasY = spawnY * tileSize + tileSize / 2;

              let stormChaser = new Sprite(canvasX, canvasY, chaserHealth, chaserHealth, "d");
...
function cameraControl() {           // taken out because its buggin the game over screen out to hell
    let baseZoom = 4;
...
function displayCoinBalance() {                                                                  //simplest bit of code ever
  if (game && !gameOver) {
...
function GO_Screen() {
  ...
  camera.zoom = 1;
  // display button
  gameOverButton = new Sprite(width / 2, height / 2, width / 3, height / 8, "k");
